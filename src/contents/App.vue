                ...
                <div v-for="rule in filteredRules" :key="rule.id" class="rule-card">
                  ...
                  <div v-show="rule.isExpanded" class="rule-card-body">
                    <div style="grid-column: span 2; display:flex; gap:8px; align-items:center;">
                      ...
                      <input type="text" v-model.lazy="rule.name" placeholder="(如: 卷名)">
                    </div>
                    <div style="grid-column: span 2; display:flex; gap:8px; align-items:center;">
                      <span style="font-size:0.85rem; opacity:0.7; width:60px;">正则式</span>
                      <input type="text" v-model.lazy="rule.pattern" placeholder="(如: 【(.*?)卷】)">
                    </div>
                    ...
                    <div v-for="rule in (settings.aiConfig.regexRules || [])" :key="rule.id" class="rule-card mini">
                      ...
                      <div v-show="rule.isExpanded" class="rule-card-body">
                        <div style="grid-column: span 2; display:flex; gap:8px; align-items:center;">
                          ...
                          <input type="text" v-model.lazy="rule.name" placeholder="如: 提取摘要">
                        </div>
                        <div style="grid-column: span 2; display:flex; gap:8px; align-items:center;">
                    ...
                    <div v-for="(msg, idx) in settings.aiConfig.promptMessages" :key="msg.id" class="prompt-msg-card" :class="[msg.role, { collapsed: !msg.isExpanded }]">
                      <div class="msg-header" @click="msg.isExpanded = !msg.isExpanded">
                        <div class="msg-title-area">
                          ...
                          <input v-model.lazy="msg.name" class="msg-name-input" placeholder="消息标题..." @click.stop>
                      ...
                      <div v-if="msg.isExpanded" class="msg-body">
                         <textarea v-model.lazy="msg.content" :placeholder="'输入指令内容...可用 {{messages}}'"></textarea>
                      </div>
                    </div>
                  </div>
                      ...
                      <div class="profile-card">
                        ...
                        <div class="input-group">
                          ...
                          <div class="model-select-area">
                            <select v-model.lazy="activeProfile.model">
...
.directory-item {
  ...
  padding-top: var(--one-c-pt, 8px);
  padding-right: var(--one-c-pr, 6px);
  padding-bottom: var(--one-c-pb, 8px);
...
.content { 
  flex: 1; 
  padding: 4px 0; /* Removed left/right padding so items can touch edges */
...
.one-h-text { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
